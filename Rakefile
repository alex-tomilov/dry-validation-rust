...
def generated_ruby_predicate_source(predicates)
...
namespace :generate do
  desc 'Generate the Ruby predicate registry from predicates.yml'
  task :predicates do
    File.write(GENERATED_RUBY_PREDICATES_PATH, generated_ruby_predicate_source(predicate_manifest))
  end

  namespace :predicates do
    desc 'Verify the generated Ruby predicate registry is in sync with predicates.yml'
    task :check do
      source = generated_ruby_predicate_source(predicate_manifest)
      unless File.exist?(GENERATED_RUBY_PREDICATES_PATH) && File.read(GENERATED_RUBY_PREDICATES_PATH) == source
        raise "#{File.basename(GENERATED_RUBY_PREDICATES_PATH)} is out of sync; run bundle exec rake generate:predicates"
      end
    end
  end
end
