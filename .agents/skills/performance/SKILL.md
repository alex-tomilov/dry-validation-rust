...
8. Update public claims only when evidence supports them. When the evidence
   changes a published performance claim or supported operational behavior,
   add a concise `Unreleased` entry derived from the measured result. If the
   optimization changes a public Ruby API, update its inline YARD documentation
   and run `bundle exec yard --fail-on-warning`.
