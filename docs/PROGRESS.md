...
The native predicate error-propagation risk is resolved, engine calls now use a typed native result, JSON plan parsing enforces its depth limit without a separate pre-scan, the project-local `benchmark-matrix` skill runs the reproducible measurement workflow, `script/verify` bootstraps its Rust advisory tool, and native-gem workflows prepare the `rb-sys-dock` Cargo registry mount and install the locked bundle in their build containers; the remaining roadmap priorities are unchanged.
