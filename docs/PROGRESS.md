...
| D   | Performance Proof       | 🔵 Active      | Phase 1 steps 1–2 and maturity-roadmap Steps 3.1–3.5 completed 2026-08-09; canonical 64-bit integers and finite decimal float parameters now coerce in Rust, while non-canonical spellings and Bignums retain Ruby behavior and explicit non-finite literals remain rejected. Type-error code/text are now borrowed static strings, eliminating their native heap allocations (2026-08-12); a three-run matrix against `HEAD` found unchanged Ruby allocations and no reliable throughput change. README records refreshed 2026-08-13 three-run results for all six scenarios, and the project-local `benchmark-matrix` skill now reproduces the matrix. CI rejects Ruby-allocation regressions above 5% of `main`, weekly CI runs five minutes of plan-deserialization fuzzing, and the Ruby suite exercises a seeded hostile-hash corpus in an isolated subprocess. JSON plan parsing now enforces its 512-container limit during Serde deserialization rather than in a separate string pre-scan (2026-08-13). ADR 002 retains JSON after three local JSON/MessagePack/direct-walking prototype runs; broader-host and native-allocation evidence remain pending |
